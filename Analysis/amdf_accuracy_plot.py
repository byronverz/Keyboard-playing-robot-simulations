...
with open("out_period_list.csv",'r') as per_file:
    periods = np.genfromtxt(per_file)
...
period_half_points = []
for i in range(len(key_pitch_list)-1):
    period_half_points.append((key_pitch_list[i]+key_pitch_list[i+1])/2)
